...
export async function uploadStaffMediaForm(formData: FormData) {
  ...
  if (!can(profile?.role, ["media_team"])) await failWarRoom("Not allowed.", "/war-room/content?tab=uploads");
  ...
  if (!title_en) await failWarRoom("Title is required.", "/war-room/content?tab=uploads");
  ...
  if (file instanceof File && file.size > 0) {
    if (file.size > 80 * 1024 * 1024) await failWarRoom("File is too large (80MB max).", "/war-room/content?tab=uploads");
    const ext = file.name.split(".").pop() ?? "bin";
    const path = `staff/${crypto.randomUUID()}.${ext}`;
    const { error: uploadError } = await sb.storage
      .from("media-public")
      .upload(path, file, { contentType: file.type, upsert: false });
    if (uploadError) await failWarRoom(uploadError.message, "/war-room/content?tab=uploads");
    storagePath = path;
    publicUrl = resolveMediaUrl(path);
  } else if (!externalUrl) {
    await failWarRoom("Add a file or a video URL.", "/war-room/content?tab=uploads");
  }
  ...
  if (error) await failWarRoom(error.message, "/war-room/content?tab=uploads");
